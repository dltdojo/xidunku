//
// [whoami/LICENSE at master · containous/whoami](https://github.com/containous/whoami/blob/master/LICENSE)
//
package main
...
import (
	...
	_ "github.com/xidunku/xhoami/statik"
)
...
func main() {
	...
	// embed static files
	http.Handle("/pub/", http.StripPrefix("/pub/", getStatikFSHandler()))

	fmt.Println("XiDunKu Starting up on port " + port)
...
func getStatikFSHandler() http.Handler {
	statikFS, err := fs.New()
	if err != nil {
		log.Fatal(err)
	}
